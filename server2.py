...
def ProduceMask(fileBytes): 
    ...
    with torch.no_grad():
        ...
        for m in range(prediction[0]['masks'].shape[0]):
            npmask = np.asarray(imgmask) | prediction[0]['masks'][m, 0].mul(255).byte().cpu().numpy()
            imgmask = Image.fromarray(npmask)
           # imgmask = Image.blend(imgmask, Image.fromarray(prediction[0]['masks'][m, 0].mul(255).byte().cpu().numpy()),0.5)
        tmp = BytesIO()
